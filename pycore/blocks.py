...
def block_Bottleneck_Drop(name, botton, s_filer=32, n_filer=(1024,1024), offset="(2,0,0)", to="(pool_b4-east)", width=(8,8), height=8, depth=8, caption="Bottleneck", opacity=0.5):
    return [
        to_Conv(name='ccr_res{}'.format(name), offset=offset, to="(ccr_{}-east)".format(botton), s_filer=s_filer, n_filer=n_filer[0], width=width[0], height=height, depth=depth),
        to_Drop(name='ccr_drop_c{}'.format(name), offset="(0,0,0)", to="(ccr_res{}-east)".format(name), s_filer="", n_filer=3, width=2, height=height, depth=depth, caption=caption),       
        to_Conv(name='ccr_{}'.format(name), offset="(0,0,0)", to="(ccr_drop_c{}-east)".format(name), s_filer=s_filer, n_filer=n_filer[1], width=width[1], height=height, depth=depth),
        to_connection( 
            "ccr_{}".format(botton), 
            "ccr_res{}".format(name)
        )
...
# simplified block
def s_block_2ConvPool( name, botton, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32,32,3.5), opacity=0.5 ):
    return [
    to_ConvConvConvRelu( 
        name="ccr_{}".format( name ),
        s_filer=str(s_filer), 
...
def s_block_Unconv( name, botton, top, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32,32,3.5), opacity=0.5 ):
    return [
